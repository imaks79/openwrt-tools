...
#
# Особенность: выбор раздела (если накопителей несколько) требует
# интерактивного ввода. Так как при запуске через "wget -O- ... | sh"
# стандартный ввод скрипта уже занят телом самого скрипта, такой `read`
# явно читает из /dev/tty — реального терминала SSH-сессии, а не из
# перекрытого stdin. Создание SMB-пользователя (ksmbd.adduser/smbpasswd)
# само запрашивает пароль напрямую через терминал и пайпа не боится.
# ==============================================================================
